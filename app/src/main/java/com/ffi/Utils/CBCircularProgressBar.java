...
/**
 * Created by Carlos Bedoy on 12/2/14.
 * Pademobile
 */
public class CBCircularProgressBar extends View {
    ...
    public CBCircularProgressBar(final Context context) {
        this(context, null);
    }

    public CBCircularProgressBar(final Context context, final AttributeSet attrs) {
        this(context, attrs, R.attr.CBCircularProgressBarStyle);
    }

    public CBCircularProgressBar(final Context context, final AttributeSet attrs, final int defStyle) {
        super(context, attrs, defStyle);

        final TypedArray attributes = context
                .obtainStyledAttributes(attrs, R.styleable.CBCircularProgressBar,
                        defStyle, 0);
        if (attributes != null)
        {
            try
            {
                setProgressColor(attributes.getColor(R.styleable.CBCircularProgressBar_progress_color, Color.CYAN));
                setProgressBackgroundColor(attributes.getColor(R.styleable.CBCircularProgressBar_progress_background_color, Color.GREEN));
                setProgress(attributes.getFloat(R.styleable.CBCircularProgressBar_progress, 0.0f));
                setWheelSize((int) attributes.getDimension(R.styleable.CBCircularProgressBar_stroke_width, 10));
                setThumbEnabled(attributes.getBoolean(R.styleable.CBCircularProgressBar_thumb_visible, true));

                mGravity = attributes.getInt(R.styleable.CBCircularProgressBar_android_gravity, Gravity.CENTER);
            }
            finally
            {
                attributes.recycle();
